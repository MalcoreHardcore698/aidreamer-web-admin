...
export default ({ status=false, act, close }) => {
    const [action, { loading }] = useMutation(EDIT_ACT)

    const [tasks, setTasks] = useState((act?.tasks)
        ? act.tasks.map(task => ({
            ...task,
    ...
    const [actAwards, setActAwards] = useState((act?.awards)
        ? act.awards.map(award => ({
            ...award,
            award: { value: award.award, label: award.award },
            quantity: award.quantity
        }))
    ...
    const [successor, setSuccessor] = useState(act?.successor)
    const [isEnableSuccessor, setEnabledSuccessor] = useState((act?.successor) ? [{ id: 0, title: 'Enable successor' }] : [])
    const [isSource, setSource] = useState((act?.isSource) ? [{ id: 0, title: 'Is Source' }] : [])
    const [_status, _setStatus] = useState(act?.status)
